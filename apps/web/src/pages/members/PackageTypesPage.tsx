...
export default function PackageTypesPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editPkg) await api.updatePlan(editPkg.id, payload);
      else await api.createPlan(payload as any);
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try { await api.deletePlan(deleteId); toast.success("Package deleted"); fetchPackages(); }
    catch { toast.error("Failed to delete"); }
    setDeleteId(null);
  };

  const toggleActive = async (pkg: PackageType) => {
    try { await api.updatePlan(pkg.id, { is_active: !pkg.is_active }); fetchPackages(); }
            ...
            <div key={pkg.id} className={`rounded-xl border bg-card p-5 transition-all ${!pkg.is_active ? "opacity-60" : ""}`}>
              ...
              <div className="flex items-center justify-between mt-4 pt-3 border-t">
                ...
                <div className="flex gap-1">
                  ...
                  <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => setDeleteId(pkg.id)}><Trash2 className="h-3.5 w-3.5" /></Button>
                </div>
              </div>
            </div>
          ))}
