...
export default function AddMemberPage() {
  ...
  useEffect(() => {
    if (!admin) return;
    supabase.from('package_types').select('*').eq('admin_id', admin.id).eq('is_active', true)
      .then(({ data }) => setPackages(data || []));
    supabase.from('members').select('id, name').eq('admin_id', admin.id).eq('is_active', true)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data: member, error } = await supabase.from('members').insert({
        ...
        reference_member_id: form.reference_member_id || null,
      }).select().single();

      if (error) throw error;

      if (form.package_type_id && endDate) {
        const { error: pkgError } = await supabase.from('member_packages').insert({
          admin_id: admin.id,
          member_id: member.id,
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Select value={form.package_type_id} onValueChange={v => { set('package_type_id', v); const p = packages.find(pkg => pkg.id === v); if (p) set('amount_paid', String(p.price)); }}>
                  ...
                  <SelectContent>
                    {packages.length === 0 ? <SelectItem value="" disabled>No packages. Add from Members &gt; Package Types.</SelectItem> : packages.map(p => <SelectItem key={p.id} value={p.id}>{p.name} — ₹{p.price}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5">
