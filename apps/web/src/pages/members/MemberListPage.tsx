...
import { Search, Plus, CreditCard as Edit, Trash2, Phone, UserX, UserCheck } from 'lucide-react';
...
export default function MemberListPage() {
  ...
  const toggleActive = async (m: Member) => {
    ...
    else { toast.success(`Member ${m.is_active ? 'deactivated' : 'activated'}`); fetchMembers(); }
  };
                    ...
                    <TableCell>
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <Button size="icon" variant="ghost" className="h-8 w-8"
                          onClick={() => toggleActive(m)} title={m.is_active ? 'Deactivate' : 'Activate'}>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete the member and all their records. This action cannot be undone.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
