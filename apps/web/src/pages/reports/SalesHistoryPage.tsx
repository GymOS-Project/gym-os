...
export default function SalesHistoryPage() {
  const { admin } = useAuth();
  const [sales, setSales] = useState<MemberPackage[]>([]);
  ...
  const filtered = sales.filter(s => {
    const matchSearch = (s.members as any)?.name?.toLowerCase().includes(search.toLowerCase()) || (s.members as any)?.phone?.includes(search);
  ...
  const statusBadge = (s: string) => {
    const cfg = { active: 'bg-green-100 text-green-700', expired: 'bg-red-100 text-red-700', cancelled: 'bg-slate-100 text-slate-700' };
    return <Badge className={cfg[s as keyof typeof cfg] || ''}>{s}</Badge>;
  };
      ...
      <div className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            <Input placeholder="Search member..." value={search} onChange={e => setSearch(e.target.value)} className="pl-3" />
          </div>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow key={s.id} className="hover:bg-muted/30">
                  <TableCell>
                    <p className="font-medium">{(s.members as any)?.name}</p>
                    <p className="text-xs text-muted-foreground">{(s.members as any)?.phone}</p>
                  </TableCell>
