...
export default function TransactionsPage() {
  ...
  const fetchTxns = async () => {
    ...
    const { data } = await supabase
      .from('transactions').select('*, members(name, phone)')
      .eq('admin_id', admin.id).order('transaction_date', { ascending: false });
    setTxns((data as any) || []);
    setLoading(false);
  };

  const filtered = txns.filter(t => {
    const name = (t.members as any)?.name?.toLowerCase() || '';
    const matchSearch = name.includes(search.toLowerCase()) || t.description?.toLowerCase().includes(search.toLowerCase());
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow key={t.id} className="hover:bg-muted/30">
                  ...
                  <TableCell>
                    <p className="font-medium text-sm">{(t.members as any)?.name || '—'}</p>
                  </TableCell>
                  <TableCell>
                    <Badge className={t.type === 'payment' ? 'bg-green-100 text-green-700' : t.type === 'refund' ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-700'}>
                      {t.type}
                    </Badge>
                  </TableCell>
