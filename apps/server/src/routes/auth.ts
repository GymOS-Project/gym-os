...
router.post("/signup", async (req, res) => {
  ...
  if (!email || !password || !gym_name || !owner_name) {
    return res.status(400).json({ message: "email, password, gym_name, and owner_name are required" });
  }
...
router.post("/login", async (req, res) => {
  ...
  res.json({ session: data.session, user: data.user, admin });
});
